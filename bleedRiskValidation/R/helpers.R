getAnalyses <- function(settings, outputFolder,cdmDatabaseName){
  ...
  cohorts <- system.file("settings", 'CohortsToCreate.csv', package = "BleedModelVal")
  ...
  if(is.null(settings)){
    ...
    settingLoc <- system.file("settings", package = "BleedModelVal")
...
getData <- function(connectionDetails,
  ...
  pathToCustom <- system.file("settings", model, package = "BleedModelVal")
  ...
  for(i in 1:nrow(measurementVarsToCreate)){
    pathToConcept <- system.file("settings", paste0(measurementVarsToCreate$covariateName[i],'_concepts.csv'), package = "BleedModelVal")
    conceptSet <- read.csv(pathToConcept)$x
    pathToScaleMap <- system.file("settings", paste0(measurementVarsToCreate$covariateName[i],'_scaleMap.rds'), package = "BleedModelVal")
  ...
  if(nrow(ageVarsToCreate)>0){
    for(i in 1:nrow(ageVarsToCreate)){
      ...
      pathToAgeMap <- system.file("settings", paste0(paste0(gsub(' ', '_',gsub('\\)','_',gsub('\\(','_',ageVarsToCreate$covariateName[i])))),'_ageMap.rds'), package = "BleedModelVal")
  ...
  if(nrow(measurementCohortVarsToCreate)>0){
    for(i in 1:nrow(measurementCohortVarsToCreate)){
      pathToConcept <- system.file("settings", paste0(measurementCohortVarsToCreate$covariateName[i],'_concepts.csv'), package = "BleedModelVal")
      conceptSet <- read.csv(pathToConcept)$x
      pathToScaleMap <- system.file("settings", paste0(measurementCohortVarsToCreate$covariateName[i],'_scaleMap.rds'), package = "BleedModelVal")
      scaleMap <- readRDS(pathToScaleMap)
